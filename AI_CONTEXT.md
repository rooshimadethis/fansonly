...
Since writes to `AppleSMC` require root privileges, the application uses a command-line helper:

- Bundled in `FanControl.app/Contents/MacOS/smc-helper`.
- Installed at `/usr/local/bin/smc-helper` when the user authorizes manual control.
- The SwiftUI app communicates with it by launching it with `Process`.
- Manual mode requires a one-time administrator prompt to install the helper with setuid permissions.
...
Prefer `./build.sh` because it sets a project-local Swift module cache path. The equivalent direct command is:

```bash
swiftc -sdk $(xcrun --show-sdk-path) -target arm64-apple-macosx14.0 -module-cache-path "$PWD/.build/module-cache" -framework Cocoa -framework SwiftUI -framework IOKit -framework ServiceManagement FanControlApp.swift MenuView.swift HelperManager.swift -o FanControl
```
